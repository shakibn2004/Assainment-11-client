...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  const links = roleLinks[user.role as keyof typeof roleLinks] || roleLinks.SUPPORTER;
    ...
    <div className="p-6 h-full flex flex-col">
      ...
      <nav className="space-y-1 flex-1">
        ...
        <div className="pt-8 mt-8 border-t border-border dark:border-white/10 space-y-1">
          <Link href="/settings" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-foreground/5 transition-colors">
            <Settings className="w-4 h-4" />
            Settings
          </Link>
          <Link href="/settings/notifications" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-foreground/5 transition-colors">
    ...
    <div className="flex-1 flex flex-col bg-background relative">
      ...
      <div className="md:hidden sticky top-20 z-40 w-full glass border-b border-border dark:border-white/10 px-4 py-3 flex items-center justify-between">
        ...
        <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
          ...
          <SheetContent side="left" className="w-[280px] p-0 glass-panel border-r border-border dark:border-white/10">
